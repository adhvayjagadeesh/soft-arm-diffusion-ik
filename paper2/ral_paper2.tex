...
single flexible backbone. The backbone is a $3.18\,$mm ($1/8''$) pultruded
fiberglass rod of free length $305\,$mm ($12''$), specified on diameter
tolerance rather than on nominal size: the disc bore is a slip fit, so stock
held to $\pm0.13\,$mm rather than $\pm0.25\,$mm is the difference between a
bore that fits every rod in the batch and one that seizes on the thick end of
it. Six spacer discs of
$56\,$mm diameter and $3.0\,$mm thickness are bonded to
...
Backbone diameter is set by a force budget rather than by convenience, and it
dominates every other mechanical choice. Bending a section to curvature
$\kappa$ requires a moment $EI\kappa$, supplied by tendon tension acting at
the outer tendon radius, and $I$ grows as the fourth power of diameter. At the
$22\,$mm moment arm used here, one servo at half stall bends a $1/8''$ rod
roughly $90$--$150^\circ$ per section but a $3/16''$ rod only $20$--$30^\circ$:
a $5.1\times$ stiffness difference that shrinks the reachable workspace by a
factor of $50$, to a $30\,$mm-deep shell in which the tip never leaves full
extension. Distinct solutions remain resolvable on the stiffer rod---they
separate by $\approx\!11\,$mm against a millimetre-scale sensing floor---so the
choice is not between a working and a broken platform, but the redundancy the
study exists to characterize is largely absent from the stiffer one.

Discs are fused-deposition printed in PLA rather than cut and drilled, for
three reasons that bear on measurement quality. The central bore is a
slip fit on the backbone, so each disc is held square by its own bore; a
drilled clearance hole in stock disc material permits several degrees of tilt
before the epoxy sets. Each disc carries an $8\,$mm collar around the bore
that extends it to $6.4\,$mm of bearing length, rather than the $3\,$mm the
disc thickness would otherwise give. The reason is quantitative: permissible
tilt is $\arctan(c/L)$ for bore clearance $c$ over bearing length $L$, so
doubling $L$ halves the angle, and a disc tilted $2.9^\circ$ displaces its
outer tendon holes by $\pm1.1\,$mm axially---comparable to the entire vision
noise floor of Sec.~\ref{sec:noise}. That displacement is \emph{random per
disc}, since each is bonded at whatever angle it happened to occupy, so
unlike a systematic bias it is not absorbed by a per-section gain fit. The
collar reduces it to $\pm0.5\,$mm for under $0.2\,$cm$^3$ of material. The marker tab is part of the disc, so its
$65^\circ$ presentation angle is identical on all six by construction rather
than by hand-gluing---and tab orientation is the single variable simultaneous
marker visibility is most sensitive to. Finally, hole positions are
reproduced from one geometric description, so disc-to-disc scatter is limited
by printer repeatability rather than by jig wear.

Each disc carries seven holes: a central clearance hole for the backbone and
six tendon guides arranged on two concentric circles at radii $12.0\,$mm and
$22.0\,$mm, with three holes per circle at $90^\circ$, $210^\circ$ and
$330^\circ$. An index notch on the rim marks the $90^\circ$ heading so that
all six discs are keyed to a common angular reference during assembly.
Section~A's three tendons run on the inner circle and terminate
...
Because all six discs are printed from one geometric description, per-disc
random error---which no calibration can absorb---is replaced by a single
systematic bias common to every disc, which one gain fit per section absorbs
exactly. \todo{report measured disc-to-disc hole scatter}

\subsection{Fabrication tolerance is measured, not assumed}
\label{sec:fab}

One fabrication detail is reported because ignoring it silently invalidates
the assembly, and because it is the difference between a platform another
group can rebuild and one that works only on the authors' printer.

Fused-deposition printers cut holes undersized by a machine-specific amount.
The figure usually quoted is $0.1$--$0.3\,$mm; on the machine used here it is
$\approx\!0.8\,$mm, three to five times larger, verified by three independent
gauge failures at nominal diameters of $2.0$, $3.3$ and $4.0\,$mm that a
single constant offset explains. An offset that size is not a finish
detail---it exceeds the slip fit being designed, so a bore drawn at nominal
prints too small to admit the backbone at all.

The offset is therefore measured with a printed gauge coupon carrying a
graded series of bores, and applied to \emph{every} hole in the part rather
than to the backbone bore alone. Compensating only the bore would leave the
tendon guides closing from $2.0\,$mm to $\approx\!1.2\,$mm, which passes the
$0.79\,$mm cable but raises the very tendon friction and hysteresis that
Sec.~\ref{sec:noise} exists to bound. \todo{report the measured offset used}

Two consequences propagate into the geometry. The compensated tendon holes
are wider, so the marker tab is positioned from the tendon diameter actually
being cut---a tab placed for nominal holes begins to overhang compensated
ones. And the backbone itself is out of round: four readings along the rod
used here span $0.10\,$mm, so the bore is sized from the \emph{largest}
measured diameter, since each disc must pass over the rod's fattest point
rather than its average.

Because this offset is a property of the machine and not of the design, it is
reported as a measured quantity alongside the geometry, and the generator
takes it as an explicit parameter rather than embedding one printer's value
...
$30\,$mm, the largest that fits the $40\,$mm printed marker tab with an
adequate quiet zone. Marker side is measured with calipers rather than taken
as nominal: monocular pose recovers range from apparent size, so a marker
declared $5\%$ large biases every reported position $5\%$ far---on this arm
roughly $15\,$mm of systematic error that averaging cannot remove and that
presents exactly as a transfer gap. Markers on discs 3 and 6 give section-boundary and tip
