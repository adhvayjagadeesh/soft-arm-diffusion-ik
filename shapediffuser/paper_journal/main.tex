...
% results_morph_3seed.json. The only remaining \todo is the author email.
...
in-distribution morphologies (three seeds)---matching a specialist trained on $5\times10^5$ samples of
...
ground-truth solution modes, as many as a specialist recovers on its own
arm. We report the single-arm advantage, its redundancy mechanism, and the
...
\begin{enumerate}
  ...
  diffusion model reaches $0.41\pm0.04\,$mm best-of-32 tip error and
  $100\%$ success on held-out in-distribution morphologies, matching a per-arm
...
and every seed (per-seed means $0.40$, $0.46$, $0.37\,$mm). On the
...
member, is $0.05\,$mm and within one standard deviation.
...
(see text). Mean $\pm$ std over three training seeds. Source:
...
arms the amortized model recovers $0.96\pm0.01$ of enumerated ground-truth
modes, with sampled diversity $25.0\pm0.2$ in curvature space---a
\emph{higher} recall than the specialist achieves on the one arm it was
trained for ($0.85\pm0.03$), and diversity in the same range
($28.1\pm0.2$). On the nominal arm itself the two are indistinguishable
($0.86\pm0.04$ vs.\ $0.85\pm0.03$). The Blind model recovers essentially
none ($0.02\pm0.01$): a model that has learned the mean arm does not merely
miss the target, it misses every mode of every arm.
...
its mode recall is $0.52\pm0.01$ and diversity $6.4\pm0.6$: it finds
\emph{a} solution before it finds \emph{all} of them. (Two of the five
extrapolation arms---all-long and all-stiff---are excluded from this
average because the enumeration itself found no modes on them: the DBSCAN
scale was calibrated to the nominal arm's curvature range, and those two
arms move furthest from it. The model reaches $97\%$ success on the stiff
arm, so this is an enumeration limit, not a model failure; it is the same
limitation noted for segment-count variation in Sec.~\ref{sec:limitations}.) Multimodality degrades before accuracy
...
and amortized-scale results are single-seed. (5) \textbf{Extrapolation is bounded.} The model
