...
| 5b | The MDN's failure is not a tuning artifact: component sweep {4,8,16,32} improves error only 18.7 -> 14.8mm (never beats the plain MLP) and mode recall stays exactly 0 at every k | `mdn_tuning_results.json` | single-seed sweep, full eval per k | SUPPORTING (one sentence in Sec IV; closes the baseline-fairness hole) |
